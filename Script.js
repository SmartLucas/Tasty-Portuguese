...
(function(){
    ...
    version = '1.3.2.4', commands = {}, tos = {}, boothcd = false, reconnect = true, inRoom = [], hover = false,
    ...
    function startup() {
        ...
        Chat('init', 'TastyPlug v' + version + ' Iniciado!<br>Digite /commands para uma lista de comandos.');
    ...
    function loadEvents() {
        ...
        $('#tp-addmention').click(function(){
            ...
            if (settings.msgs.length > len) {
                Chat('info', 'Palavra(s) Adicionadas a lista de menções');
                saveSettings();
            }
        });
        $('#tp-delmention').click(function(){
            ...
            if (settings.msgs.indexOf(a) > -1) {
                ...
                saveSettings();
            } else Chat('error', 'Esta palavra não esta na lista de menções!');
        });
        $('#tp-listmention').click(function(){
            ...
            return Chat('error', 'Você não tem nada em sua lista de menções');
        });
    ...
    function eventWLUpd() {
        ...
        if (settings.boothalert && API.getWaitListPosition() < 3 && API.getWaitListPosition() != -1 && !boothcd) {
            chatSound();
            Chat('info','[Aviso] É quase a sua vez de DJ! Certifique-se de escolher uma música!');
            boothcd = true;
        }
    }
        ...
        if (settings.histalert && getRank(API.getUser()) >= 2 && a.media) {
            ...
            for (var i = 0; i < hist.length; i++) {
                if (hist[i].media.cid == a.media.cid) {
                    Chat('error','Esta música está no histórico! (played ' + (i + 1) + ' song' + (i == 0 ? '' : 's') + ' ago)');
                    chatSound();
                    break;
                }
            }
    ...
    commands.ban = function(a) {
        ...
        if (getRank(API.getUser()) <= getRank(user)) return Chat('error', 'Você não pode banir as pessoas que são de grau igual ou maior que você!');
    ...
    commands.kick = function(a) {
        ...
        if (!user) return Chat('error', 'User not found.');
        if (getRank(API.getUser()) <= getRank(user)) return Chat('error', 'Você não pode kickar as pessoas que são de grau igual ou maior que você!');
    ...
    commands.pm = function(a) {
        ...
        if (!user) return Chat('error', 'User not found.');
        if (user.id == API.getUser().id) return Chat('error', 'Você não pode mandar um PM para si mesmo!');
        ...
        return true;
        */
        Chat('error', 'PMs estão temporariamente desativados! Desculpe!');
    };
    commands.r = function(a) {
        if (settings.lastPM) eventCommand('/pm @' + settings.lastPM + ' ' + a.message.split(' ').slice(1).join(' '));
        else Chat('error', 'Você não recebeu um PM ainda!');
    };
